...
def safe_division(numerator, denominator, fallback=0.0):
    ...
    try:
        ...
        # Verificar se o resultado é válido para JSON
...
def safe_float(value, fallback=0.0):
    ...
    try:
        ...
        # Verificar se é válido para JSON
        if math.isnan(result) or math.isinf(result):
            return fallback
...
def get_m2_global_momentum():
    """M2 Global Momentum Score - VERSÃO SEGURA"""
    try:
...
def _get_m2_from_notion():
    """Busca M2 do Notion - VERSÃO SEGURA"""
    try:
        from notion_client import Client
        settings = get_settings()
        notion = Client(auth=settings.NOTION_TOKEN)
